...
import { Search, MapPin,Building2,Gauge,Package, Calendar, User, Phone, Mail, FileText, PhoneCall } from 'lucide-react';
...
const EquipmentCards  = () => {
       ...
       useEffect(()=>{
             ...
             // alert()
               setCitiesLoading(true);
                let url = server_url+"/user/fetchcities";
  ...
  const searchMedicines = async (city, equipmentName) => {
           if(city=='' || equipmentName==''){
            return -1;
           }
           setLoading(true)
  };
  ...
  const handleFetch = async() => {
    if (!selectedCity && !medicineName) {
      alert('Please select a city or enter a equipment name');
      return;
    }
  let ans=await searchMedicines(selectedCity, medicineName);
     ...
     // alert(selectedCity+"  "+medicineName)
      let formdata=new FormData()
         ...
         formdata.append("equipmentName",medicineName)
      let url = server_url+"/user/fetchequipmentData";
   ...
   // alert(number)
       localStorage.setItem('contact',number)
          navigate('/needy-navbar/contact-page2')
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-400 via-purple-400 to-pink-200 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-white mb-2">Equipment Search</h1>
          <p className="text-white text-lg opacity-90">Find available equipment in your city</p>
        </div>
              ...
              {jsonary.map((medicine) => (
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-xl font-bold">EquipmentName: {medicine.equipmentName}</h3>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                      <Gauge size={16} />
                      <span className="text-sm">Euantity: {medicine.quantity}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <FileText size={16} />
                      <span className="text-sm">description: {medicine.description}</span>
                    </div>
                    ...
                    <div className="flex gap-2 pt-2">
                      <a
                          onClick={()=>getcontact(medicine.number)} 
